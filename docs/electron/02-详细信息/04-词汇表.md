...
IPC 代表 Inter-Process Communication进程间通信。Electron 使用 IPC 来在[main主进程](https://www.electronjs.org/docs/glossary#main-process)和[renderer渲染进程](https://www.electronjs.org/docs/glossary#renderer-process)之间传递 JSON 信息。

### libchromiumcontent

包含 [Chromium Content module ](https://www.chromium.org/developers/content-module)及其所有依赖项 (例如, Blink、[ V8 ](https://www.electronjs.org/docs/glossary#v8)等) 的共享链接库。 也称为 “libcc”。
...
参见: [process](https://www.electronjs.org/docs/glossary#process), [renderer process](https://www.electronjs.org/docs/glossary#renderer-process)

### MAS

是指苹果系统上的 Mac App Store 的缩略词。有关于如何提交你的 app 至 MAS ，详见 [Mac App Store Submission Guide](https://www.electronjs.org/docs/tutorial/mac-app-store-submission-guide)。
...
参见： [Using Native Node Modules](https://www.electronjs.org/docs/tutorial/using-native-node-modules).
...
一个进程是计算机程序执行中的一个实例。 Electron 应用同时使用了 [main(主进程)](https://www.electronjs.org/docs/glossary#main-process) 和一个或者多个 [rendere(渲染进程)](https://www.electronjs.org/docs/glossary#renderer-process) 来运行多个程序。

在 Node.js 和 Electron 里面，每个运行的进程包含一个 `process `对象。 这个对象作为一个全局的提供当前进程的相关信息和操作方法。 作为一个全局变量，它在应用内能够不用 require() 来随时取到。

参见： [main process](https://www.electronjs.org/docs/glossary#main-process), [renderer process](https://www.electronjs.org/docs/glossary#renderer-process)
...
参见： [process](https://www.electronjs.org/docs/glossary#process), [main process](https://www.electronjs.org/docs/glossary#main-process)

### Squirrel

Squirrel 是一个开源的框架来让 Electron 的应用能够自动的更新到发布的新的版本。 详见 [autoUpdater](https://www.electronjs.org/docs/api/auto-updater) API 了解如何开始使用 Squirrel。
...
- [docs/development/v8-development.md](https://www.electronjs.org/docs/development/v8-development)
